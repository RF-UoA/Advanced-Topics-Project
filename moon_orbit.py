...
def create_extrinsic_matrix(plane_normal, radius, rot_angle_range=0, rotate=False, att_noise_range=None):
...
if __name__ == "__main__":
    ...
    # coe_oe = np.array([7000, 0.1, np.deg2rad(30), np.deg2rad(40), np.deg2rad(60), np.deg2rad(45)])

    # APOLLO ORBIT (ish)
    coe_oe = np.array([1837, 0.01, 90.0, 45.0, 0.0, 0.0])
    ...
    # mu_earth = 398600
    # rv = KeprvTrans.coe2rv(coe_oe, mu_earth)

    print("State Vector:")
    print(f"Position: {rv[:3]} km")
    print(f"Velocity: {rv[3:]} km/s")
